...
class MachineFormState {
  ...
  Map<String, dynamic> toMap(Map<String, Map<String, dynamic>> extrasMap) {
    return {
      ...
      'severity': severity,
      'items': items
          .map((e) => {...e.toMap(), ...(extrasMap[e.itemId] ?? {})})
          .toList(),
...
class CreateServiceQuotationScreen extends StatefulWidget {
  ...
  @override
  State<CreateServiceQuotationScreen> createState() =>
      _CreateServiceQuotationScreenState();
}

class _CreateServiceQuotationScreenState
    extends State<CreateServiceQuotationScreen> {
  ...
  final ValueNotifier<List<ext_models.VisitCharge>> _visitCharges =
      ValueNotifier<List<ext_models.VisitCharge>>([]);
  final ValueNotifier<List<TermRow>> _dynamicTerms =
      ValueNotifier<List<TermRow>>([]);
  final ValueNotifier<QuotationTotals> _globalTotals =
      ValueNotifier<QuotationTotals>(QuotationTotals.zero);
  final ValueNotifier<Map<String, dynamic>?> _customerInsights =
  ...
  final TextEditingController _quotationSequenceController =
      TextEditingController();
  final TextEditingController _serviceRequestNumberController =
      TextEditingController();
  final TextEditingController _subjectController = TextEditingController();
  final TextEditingController _followUpNotesController =
      TextEditingController();
  final TextEditingController _serviceRefNoteController =
      TextEditingController();
  ...
  DocumentReference? get _companyDoc =>
      _companyId != null ? _db.collection('companies').doc(_companyId) : null;
  CollectionReference? get _quotesColl =>
      _companyDoc?.collection('service_quotations');
  CollectionReference? get _customersColl =>
      _companyDoc?.collection('customers');
  CollectionReference? get _productsColl => _companyDoc?.collection('products');
  CollectionReference? get _requestsColl =>
      _companyDoc?.collection('service_requests');
  ...
  void initState() {
    ...
    _allControllers = [
      ...
      _gstController,
      _quotationSequenceController,
      _serviceRequestNumberController,
      _subjectController,
      _followUpNotesController,
      _serviceRefNoteController,
      _signNameController,
      _signDesignationController,
      _signPhoneController,
    ];

    _machine = ValueNotifier<MachineFormState>(
      MachineFormState(
        machineUid: _uuid.v4(),
        model: '',
        serial: '',
  ...
  void dispose() {
    ...
    for (var ctrl in _allControllers) {
      ctrl.dispose();
    }
    for (var term in _dynamicTerms.value) {
      term.dispose();
    }
    _isLoading.dispose();
    _isReadOnly.dispose();
  ...
  Future<void> _loadUserContext() async {
    ...
    if (_companyDoc != null) {
      final compUserDoc = await _companyDoc!
          .collection('users')
          .doc(user.uid)
          .get();
      final compData = compUserDoc.exists ? compUserDoc.data() ?? {} : {};
      _currentUserName = (compData['name'] ?? rootData['name'] ?? '')
          .toString()
          .trim();
      ...
      if (widget.existingQuotation == null) {
        ...
        _signDesignationController.text = _normalizeSignatoryDesignation(
          _currentUserName,
          (compData['designation'] ?? _currentUserRole)
              .toString()
              .toUpperCase(),
        );
        _signPhoneController.text =
            (compData['phone'] ?? user.phoneNumber ?? '').toString();
      }
    }
  }
  ...
  Future<void> _loadCompanyProfile() async {
    ...
    _quotationPrefix =
        (data['quotationPrefix'] ?? fallbackQuotationSeriesPrefix)
            .toString()
            .toUpperCase();
  ...
  Future<void> _loadUserSettings() async {
    ...
    if (doc.exists && doc.data() != null) {
      _packingChargesExtra.value =
          doc.data()!['packingChargesExtra'] as bool? ?? true;
    }
  }
  ...
  void _applyProfessionalDefaultTerms() {
    _dynamicTerms.value = [
      TermRow(
        id: _uuid.v4(),
        title: 'Payment',
        value: '100% advance against Proforma Invoice before dispatch/visit.',
      ),
      TermRow(
        id: _uuid.v4(),
        title: 'Validity',
      ...
      TermRow(
        ...
        title: 'Service Terms',
        value:
            'Boarding, lodging and local transport of engineer to be arranged by the buyer.',
      ),
  ...
  Future<void> _fetchNextSequencePreview() async {
    ...
    try {
      final counterRef = _companyDoc!
          .collection('counters')
          .doc('service_quotation_counter_$financialYear');
  ...
  String _deriveWarranty(Map<String, dynamic> machine) {
    ...
    if (amcEnd != null)
      return amcEnd.isAfter(now) ? 'AMC Active' : 'AMC Expired';
    if (wEnd != null)
      return wEnd.isAfter(now) ? 'Under Warranty' : 'Out Of Warranty';
    return (machine['isWarranty'] == true || machine['isWarranty'] == 'true')
        ? 'Under Warranty'
        : 'Out Of Warranty';
  }
  ...
  String _deriveSeverity(Map<String, dynamic> machine) {
    ...
    final cat =
        (machine['complaintCategory'] ?? machine['serviceCategory'] ?? '')
            .toString()
            .toLowerCase();
  ...
  Future<void> _loadExistingQuotation(Map<String, dynamic> data) async {
    _approvalStatus =
        data['approvalStatus']?.toString() ?? 'Waiting Customer Approval';
    ...
    if ((_approvalStatus == 'Approved' ||
            _quotationStatus == 'Converted To Work Order') &&
        !_isAdminOrManager) {
      _isReadOnly.value = true;
    }

    _quoteDate =
        _parseSafeDate(data['quoteDate'])?.toUtc() ?? DateTime.now().toUtc();
    ...
    if (quoteParts.length >= 3) {
      ...
      _quotationSequenceController.text = quoteParts.length > 1
          ? quoteParts[1]
          : '';
    }

    _subjectController.text =
        data['remarks']?.toString() ?? data['subject']?.toString() ?? '';
    ...
    if (_selectedCustomerId != null) {
      await _loadCustomerFromFirestore(
        _selectedCustomerId!,
        skipOverrides: true,
      );
    }
    ...
    _selectedRequestSource =
        data['requestSource']?.toString() ??
        data['quotationSource']?.toString() ??
        'Verbal';
    _requestDate =
        _parseSafeDate(data['requestDate'])?.toUtc() ?? DateTime.now().toUtc();
    _serviceRefNoteController.text = data['serviceReference']?.toString() ?? '';

    _signNameController.text =
        data['signatureName']?.toString() ?? _signNameController.text;
    _signDesignationController.text = _normalizeSignatoryDesignation(
      _signNameController.text,
    ...
    _signPhoneController.text =
        data['signaturePhone']?.toString() ?? _signPhoneController.text;
    ...
    if (loadedModel.isEmpty &&
        data['machines'] != null &&
        (data['machines'] as List).isNotEmpty) {
      loadedModel =
          (data['machines'] as List)[0]['machineModel']?.toString() ?? '';
    }
    ...
    if (data['visitCharges'] != null && data['visitCharges'] is List) {
      _visitCharges.value = (data['visitCharges'] as List)
          .map(
            (e) => ext_models.VisitCharge.fromMap(Map<String, dynamic>.from(e)),
          )
          .toList();
    }
  ...
  Future<void> _applyServiceRequestSeedIfNeeded() async {
    ...
    _linkedServiceRequestId =
        seed['id']?.toString() ?? seed['serviceRequestId']?.toString();
    _linkedServiceRequestNumber =
        seed['serviceRequestNumber']?.toString() ??
        seed['requestNumber']?.toString();
    _serviceRequestNumberController.text = _linkedServiceRequestNumber ?? '';
    _selectedRequestSource = 'Service Request';

    _requestDate =
        _parseSafeDate(seed['requestDate'])?.toUtc() ??
        _parseSafeDate(seed['createdAt'])?.toUtc() ??
        DateTime.now().toUtc();

    _clientNameController.text =
        (seed['customerName'] ?? seed['companyName'] ?? '').toString().trim();
    _contactPersonController.text = (seed['contactPerson'] ?? '')
        .toString()
        .trim();
    _emailController.text = (seed['email'] ?? '').toString().trim();
    _mobileController.text = (seed['mobileNumber'] ?? seed['mobile'] ?? '')
        .toString()
        .trim();
    ...
    _subjectController.text =
        (seed['subject'] ??
                seed['requestSubject'] ??
                'Quotation for Service/Spares')
            .toString()
            .trim();

    final notes =
        (seed['notes'] ?? seed['description'] ?? seed['serviceReference'] ?? '')
            .toString()
            .trim();
    ...
    if (combinedNotes.isNotEmpty)
      _serviceRefNoteController.text = combinedNotes.join('\n');
    ...
    _machine.value = MachineFormState(
      machineUid: _uuid.v4(),
      model: (seed['machineModel'] ?? seed['serviceItemName'] ?? '')
          .toString()
          .trim(),
      ...
      serviceCategory:
          seed['complaintCategory'] ?? seed['serviceCategory'] ?? 'General',
  ...
  Future<List<ext_models.QuotationLineItem>> _hydrateItems(
    List rawItems,
  ) async {
  ...
  Future<ext_models.QuotationLineItem?> _hydrateSingleItem(
    Map<String, dynamic> i,
  ) async {
    String productId = (i['productId'] ?? i['partId'] ?? i['itemId'] ?? '')
        .toString();
    String name =
        (i['name'] ?? i['itemName'] ?? i['partName'] ?? i['productName'] ?? '')
            .toString();
    double qty =
        double.tryParse(
          i['quantity']?.toString() ?? i['qty']?.toString() ?? '1',
        ) ??
        1.0;
    double price =
        double.tryParse(
          i['unitPrice']?.toString() ?? i['rate']?.toString() ?? '0',
        ) ??
        0.0;
    double disc =
        double.tryParse(
    ...
    double totalGst =
        double.tryParse(
          i['gstPercentage']?.toString() ?? i['tax']?.toString() ?? '18',
        ) ??
        18.0;
      ...
      if (doc.exists && doc.data() != null) {
        ...
        if (!skipOverrides) {
          ...
          if (_selectedContactId == null) {
            final primaryContact = _customerContacts.isNotEmpty
                ? _customerContacts.first
                : null;
            if (primaryContact != null) {
              ...
              _contactPersonController.text =
                  (data['contactPerson'] ?? data['contactName'] ?? '')
                      .toString();
              _contactDesignationController.text = (data['designation'] ?? '')
                  .toString();
              ...
              _contactDesignationSnapshot = _contactDesignationController.text
                  .trim();
            }
          }
        }
  ...
  Future<void> _fetchCustomerInsights(String custId) async {
    ...
    try {
      ...
      if (snaps.docs.isNotEmpty) {
        lastQuote =
            double.tryParse(
              (snaps.docs.first.data() as Map<String, dynamic>)['finalTotal']
                      ?.toString() ??
                  '0',
            ) ??
            0.0;
        for (var d in snaps.docs) {
          totalVal +=
              double.tryParse(
                (d.data() as Map<String, dynamic>)['finalTotal']?.toString() ??
                    '0',
              ) ??
              0.0;
        }
      }
      _customerInsights.value = {
                  ...
                  onChanged: (value) {
                    ...
                    debounce = Timer(
                      ...
                      () async {
                        ...
                        try {
                          if (_customersColl == null)
                            throw Exception("No company context");
  ...
  void _onItemsChanged(List<ext_models.QuotationLineItem> extItems) {
    ...
    for (var ext in extItems) {
      final existingMatches = _machine.value.items.where(
        (i) => i.itemId == ext.itemId,
      );
      final existing = existingMatches.isNotEmpty
          ? existingMatches.first
          : null;
      if (existing != null) {
        newInternalItems.add(
          existing.copyWith(
            qty: ext.qty,
            rate: ext.rate,
  ...
  void _checkInterState() {
    _isInterState =
        _companyState.isNotEmpty &&
        _customerState.isNotEmpty &&
        _companyState != _customerState;
    _recalculateTaxes();
  }
  ...
  void _recalculateTaxes() {
    ...
    for (var item in currentMachine.items) {
      ...
      double totalGst =
          (double.tryParse(extras['cgstPercent']?.toString() ?? '0') ?? 0.0) +
          (double.tryParse(extras['sgstPercent']?.toString() ?? '0') ?? 0.0) +
          (double.tryParse(extras['igstPercent']?.toString() ?? '0') ?? 0.0);

      if (totalGst == 0)
        totalGst = double.tryParse(extras['baseGst']?.toString() ?? '0') ?? 0.0;
  ...
  int get _totalItemsCount =>
      _machine.value.items.length + _visitCharges.value.length;
  ...
  bool _validateBeforeSave() {
    ...
    if (qDate.isBefore(reqDate)) {
      _showSnack(
        'Quotation date cannot be earlier than Request date.',
        isError: true,
      );
      return false;
    }
  ...
  Future<void> _ensureExistingQuoteNumberIsUnique(String quoteNumber) async {
    ...
    if (duplicateExists)
      throw Exception('Quotation number $quoteNumber already exists.');
  }
    ...
    try {
      ...
      if (manualSequenceStr.isNotEmpty) {
        ...
        if (manualSequence == null)
          throw Exception('Invalid sequence number format.');
        await _ensureExistingQuoteNumberIsUnique(
          '$_quotationPrefix/SQ/${manualSequence.toString().padLeft(3, '0')}/$financialYear',
        );
      }
      ...
      if (!isUpdate || isRevision) {
        ...
        payload['version'] = isRevision
            ? _currentVersion + 1
            : (isUpdate ? _currentVersion : 1);
        payload['isLatest'] = true;
        payload['parentQuotationId'] = isRevision ? widget.quotationId : null;
      }

      bool autoAdjusted = await _executeSaveTransaction(
        db,
        docRef,
        payload,
        isRevision,
      ...
      if (!_isReadOnly.value) {
        await db.collection('quotationSettings').doc(_currentUserUid).set({
          'dynamicTerms': _dynamicTerms.value
              .map(
                (e) => {
                  'id': e.id,
      ...
      if (mounted) {
        if (autoAdjusted) {
          _showSnack(
            'Quotation number already exists. Next available number selected.',
            isError: false,
          );
        } else {
          _showSnack(
            isRevision ? 'Revision Created!' : 'Service Quotation Saved!',
          );
        }
  ...
  Future<bool> _executeSaveTransaction(
      ...
      if (isRevision) {
        ...
        if (manualSequence == null) {
          ...
          for (int attempt = 0; attempt < 10000; attempt++) {
            safeQuoteNumber =
                '$_quotationPrefix/SQ/${nextSequence.toString().padLeft(3, '0')}/$financialYear';
            final numberRef = _companyDoc!
                .collection('service_quotation_numbers')
                .doc(_quoteNumberRegistryId(safeQuoteNumber));
            final numberDocSnap = await tx.get(numberRef);
            final reservedFor = numberDocSnap
                .data()?['quotationId']
                ?.toString();

            if (!numberDocSnap.exists ||
                (reservedFor == null) ||
                allowedReservations.contains(reservedFor)) {
  ...
  Map<String, dynamic> _buildQuotationData(String docId) {
    ...
    final flatItems = machineState.items
        .map((e) => {...e.toMap(), ...(_itemExtras[e.itemId] ?? {})})
        .toList();
    ...
    return {
      ...
      'quotationSource': _selectedRequestSource == 'Service Request'
          ? 'Service Request'
          : 'Manual',
      ...
      'taxAmount':
          _globalTotals.value.cgst +
          _globalTotals.value.sgst +
          _globalTotals.value.igst,
  ...
  ext_models.ServiceQuotationModel _buildExternalModel() {
    return ext_models.ServiceQuotationModel(
      ...
      quotationNumber: _fullExistingQuoteNumber.isNotEmpty
          ? _fullExistingQuoteNumber
          : _getLiveQuoteNumberDisplay(),
      requestId: _linkedServiceRequestId ?? '',
      requestNumber: _serviceRequestNumberController.text,
      visitId: '',
      visitNumber: '',
      customerId: _selectedCustomerId ?? '',
      customerName: _clientNameController.text.isNotEmpty
          ? _clientNameController.text
          : 'Unknown',
  ...
  void _previewPdf() {
    bool hasAnyItem =
        _visitCharges.value.isNotEmpty || _machine.value.items.isNotEmpty;
    if (!hasAnyItem) {
      _showSnack('Add items before previewing.', isError: true);
      return;
    }
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => Scaffold(
          ...
          body: PdfPreview(
            build: (format) =>
                ServiceQuotationPdfGenerator.generateServiceQuotationPdf(
                  _buildPreviewData(),
                  items,
                ),
            allowPrinting: true,
  ...
  void _openRequest() {
    if (_linkedServiceRequestId != null &&
        _linkedServiceRequestId!.isNotEmpty) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'This document is $_approvalStatus and locked.',
                  style: TextStyle(
      ...
      builder: (context, machineState, _) {
        return ValueListenableBuilder<QuotationTotals>(
          ...
          builder: (context, totals, _) {
            final customerName = _clientNameController.text.isNotEmpty
                ? _clientNameController.text
                : 'Unknown Customer';
            final machineModel = machineState.model.isNotEmpty
                ? machineState.model
                : 'No Machine Selected';
            ...
            Align(
              ...
              child: OutlinedButton.icon(
                onPressed: () async {
                  ...
                  if (c != null && c['id'] != null)
                    await _loadCustomerFromFirestore(c['id']);
                },
          ...
          child: Column(
            ...
            children: [
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        ...
                        isDense: true,
                        filled: true,
                        fillColor: _isReadOnly.value
                            ? Colors.grey.shade50
                            : Colors.white,
                      ),
                      onChanged: (v) =>
                          _updateMachine(machineState.copyWith(serial: v)),
                    ),
                  ),
                  const SizedBox(width: 16),
                  Expanded(
              ...
              TextFormField(
                ...
                maxLines: 2,
                decoration: InputDecoration(
                  labelText: 'Complaint / Requirement',
                  labelStyle: const TextStyle(
                    fontWeight: FontWeight.w500,
                    color: textMuted,
                  ),
                ...
                onChanged: (v) =>
                    _updateMachine(machineState.copyWith(complaint: v)),
              ),
            ],
          ),
        );
            ...
            children: [
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    if (totals.cgst > 0)
                      _buildSummaryRowItem('CGST', totals.cgst),
                    if (totals.sgst > 0)
                      _buildSummaryRowItem('SGST', totals.sgst),
                    if (totals.igst > 0)
                      _buildSummaryRowItem('IGST', totals.igst),
              ...
              ValueListenableBuilder<bool>(
                ...
                builder: (context, val, _) => SwitchListTile(
                  ...
                  subtitle: const Text(
                    'Add remark that packing & transport are extra at actuals',
                  ...
                  onChanged: _isReadOnly.value
                      ? null
                      : (newVal) => _packingChargesExtra.value = newVal,
                ),
              ),
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              ...
              color: isGrandTotal
                  ? Colors.green
                  : (isDeduction ? Colors.red : textDark),
            ),
          ),
        ],
      ),
    ...
    return _buildSectionExpansionTile(
      ...
      child: Column(
        ...
        children: [
          ValueListenableBuilder<bool>(
            valueListenable: _isReadOnly,
            builder: (ctx, readOnly, _) => readOnly
                ? const SizedBox.shrink()
                : OutlinedButton.icon(
                    onPressed: () {
                      final currentTerms = List<TermRow>.from(
                        _dynamicTerms.value,
                      );
            ...
            builder: (context, readOnly, _) {
              ...
              return TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  isDense: true,
                  filled: true,
                  fillColor: effReadOnly ? Colors.grey.shade50 : Colors.white,
                ),
              );
            },
          ),
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (ctx, snap) {
        ...
        if (machine.categoryId != null &&
            !items.any((e) => e.value == machine.categoryId)) {
          items.insert(
            0,
            DropdownMenuItem(
              value: machine.categoryId,
              child: const Text('Unknown Category'),
            ),
          );
        }
        ...
        return DropdownButtonFormField<String>(
          ...
          decoration: InputDecoration(
            ...
            isDense: true,
            filled: true,
            fillColor: _isReadOnly.value ? Colors.grey.shade50 : Colors.white,
          ),
          ...
          onChanged: _isReadOnly.value
              ...
              : (val) {
                  _updateMachine(
                    machine.clearCascade(1).copyWith(categoryId: val),
                  );
                },
        );
      },
  ...
  Widget _buildSubcategoryDropdown(MachineFormState machine) {
    return StreamBuilder<QuerySnapshot>(
      stream: machine.categoryId != null
          ? _companyDoc
                ?.collection('inventory_categories')
                .doc(machine.categoryId)
      ...
      builder: (ctx, snap) {
        ...
        if (machine.subcategoryId != null &&
            !items.any((e) => e.value == machine.subcategoryId)) {
          items.insert(
            0,
            DropdownMenuItem(
              value: machine.subcategoryId,
              child: const Text('Unknown Subcategory'),
            ),
          );
        }
        ...
        return DropdownButtonFormField<String>(
          ...
          onChanged: _isReadOnly.value
              ...
              : (val) {
                  _updateMachine(
                    machine.clearCascade(2).copyWith(subcategoryId: val),
                  );
                },
        );
      },
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (ctx, snap) {
        ...
        if (machine.machineType != null &&
            !items.any((e) => e.value == machine.machineType)) {
          items.insert(
            0,
            DropdownMenuItem(
              value: machine.machineType,
              child: Text(machine.machineType!),
            ),
          );
        }
        ...
        return DropdownButtonFormField<String>(
          ...
          onChanged: _isReadOnly.value
              ...
              : (val) {
                  _updateMachine(
                    machine.clearCascade(3).copyWith(machineType: val),
                  );
                },
        );
      },
  ...
  Widget _buildMachineModelDropdown(MachineFormState machine) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (ctx, snap) {
        ...
        if (machine.productId != null &&
            !items.any((e) => e.value == machine.productId)) {
          items.insert(
            0,
            DropdownMenuItem(
          ...
          onChanged: _isReadOnly.value
              ...
              : (val) {
                  if (val != null && snap.hasData) {
                    ...
                    _updateMachine(
                      machine.copyWith(productId: val, model: modelName),
                    );
                  }
                },
        );
