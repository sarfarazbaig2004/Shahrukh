...
class _TaskScreenState extends State<TaskScreen> {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> _taskStream() {
    final base = _db.collection('companies').doc(widget.companyId).collection('tasks');
    if (isAdmin) return base.snapshots();
    return base.where('assignedToUids', arrayContains: widget.currentUserId).snapshots();
  }
          ...
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snap) {
              if (snap.hasError) {
                return Expanded(
                  child: Center(
                    child: Text('Unable to load tasks: ${snap.error}', textAlign: TextAlign.center),
                  ),
                );
              }
              if (!snap.hasData) {
